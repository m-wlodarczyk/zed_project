...
treatment_duration_stats <- patients %>%
  group_by(age_group) %>%
  summarise(Mean=mean(treatment_duration), Max=max(treatment_duration), Min=min(treatment_duration), Std=sd(treatment_duration), .groups = "rowwise")

treatment_duration_stats %>% ggplot(aes(x=age_group, y=Mean, group=1)) +
