...
Pacjenci byli poddawani badaniom ze średnią częstotliwością `r mean(patients$tests_freq)`

```{r tests_freq_plot, fig.align='center'}
...
lymphocytes_tests <- df %>% 
  ...
  mutate(test_performed=row_number(patient_id)) %>%
  group_by(test_performed, survived) %>%
  summarise(lymphocytes=mean(lymphocytes))
...
#anim_save("LDH.gif", ldh_animated_plot)
#anim_save("hs-CRP.gif", hs_crp_animated_plot)
#anim_save("lymphocytes.gif", lymphocytes_animated_plot)
```
